...
from spark.query.allQuery import build_month_dataframe
from spark.query.dashboard_analysis.avg_analysis_flights import mese_num_voli_medie_ritardo_distanza_minutiVolo
from spark.query.dashboard_analysis.cause_ritardi_voli import causeRitardo_numVoli
from spark.query.dashboard_analysis.info_voli import informazioni_mensili
from spark.query.dashboard_analysis.range_ritardo import rangeRitardi
from spark.utils.utils import mese_da_nome, mese_precedente
...
if mese:
    df_mese= build_month_dataframe(mese_da_nome(mese))
    lista_avg = mese_num_voli_medie_ritardo_distanza_minutiVolo(df_mese)
    ...
    if(mese=="gennaio"):
        a.metric("Numero voli", lista_avg[0], delta=0.0,  border=True)
        b.metric("Ritardo medio", f"{lista_avg[1]} min", delta=0.0,  border=True)
        c.metric("Distanza media", f"{lista_avg[2]} miglia", delta=0.0, border=True)
        d.metric("Media minuti di volo", f"{lista_avg[3]} min", delta=0.0, border=True)


    else:
        df_mese_precedente = build_month_dataframe(mese_da_nome(mese_precedente(mese)))
        lista_avg_precedente = mese_num_voli_medie_ritardo_distanza_minutiVolo(df_mese_precedente)
    ...
    range_ritardi = rangeRitardi(df_mese)
    ...
    with col2:
        ...
        info_voli_mese = informazioni_mensili(df_mese)
    ...
    cause_ritardi= causeRitardo_numVoli(df_mese)
