...
from spark.query.allQuery import query_numero_partenze_e_arrivi_citta, \
    query_ritardo_medio_partenza_arrivo_citta, query_destinazione_numvoli_citta, query_citta_numvoli_aeroporto
from spark.utils.utils import getListaCitta, get_airport_coordinates, getCordinateCitta_asDict, getSortedListaCitta
...
def create_dataframe_pandas_to_visualize_city_coordinates(lista_citta):
    ...
    latitudine=[]
    longitudine=[]
    dict_citta_coordinate= getCordinateCitta_asDict()
...
def create_dataframe_pandas_to_visualize_map_airports_coordinates(aeroporti):
    """ Crea un DataFrame Pandas con le coordinate degli aeroporti forniti. """
    airport_name = []
    airport_lat = []
    airport_lon = []
...
def create_dataframe_pandas_to_visualize_nmaxdestfrequenti(dest_numvoli, nummaxfrequenti=5):
     ...
     dict_ord = sorted(dest_numvoli.items(), key=lambda x: x[1], reverse=True)[:nummaxfrequenti] #[(K,V),[(K1,V1)]]
