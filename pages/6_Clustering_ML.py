...
from spark.query.allQuery import build_month_dataframe, clusteringFlights, preprocessing_clustering
from spark.utils.utils import mese_da_nome
...
st.write("Questa pagina mostra la visualizzazione del clustering K-means per diverse configurazioni di 'k'")

mese = st.sidebar.selectbox('Seleziona un mese',mesi,index=None)

if mese:

    df = build_month_dataframe(mese_da_nome(mese))
